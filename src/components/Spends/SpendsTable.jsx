...
const SpendsTable = ({
  ...
  const getCategoryConfig = (category) => {
    const configs = {
      Decoration:    { gradient: "from-primary to-secondary",   bgLight: "bg-primary/10",   border: "border-primary/20",   text: "text-primary"         },
      Food:          { gradient: "from-secondary to-accent",    bgLight: "bg-secondary/10", border: "border-secondary/20", text: "text-secondary"       },
      Equipment:     { gradient: "from-accent to-info",         bgLight: "bg-accent/10",    border: "border-accent/20",    text: "text-accent"          },
      Media:         { gradient: "from-info to-primary",        bgLight: "bg-info/10",      border: "border-info/20",      text: "text-info"            },
      Transport:     { gradient: "from-success to-emerald-600", bgLight: "bg-success/10",   border: "border-success/20",   text: "text-success"         },
      Entertainment: { gradient: "from-warning to-orange-500",  bgLight: "bg-warning/10",   border: "border-warning/20",   text: "text-warning"         },
      Other:         { gradient: "from-base-300 to-base-400",   bgLight: "bg-base-300/30",  border: "border-base-300",     text: "text-base-content/70" },
    };
  ...
  const getStatusConfig = (status) => {
    const configs = {
      pending:  { bg: "bg-warning/20", text: "text-warning", border: "border-warning/30", dot: "bg-warning" },
      approved: { bg: "bg-success/20", text: "text-success", border: "border-success/30", dot: "bg-success" },
      rejected: { bg: "bg-error/20",   text: "text-error",   border: "border-error/30",   dot: "bg-error"   },
    };
      ...
      <div className="flex items-center gap-2">
        ...
        {sortBy === key && (
          <div className="flex flex-col">
            <ArrowUpIcon className={`h-3 w-3 transition-all ${sortOrder === "asc" ? "text-primary scale-110" : "text-base-content/30"}`} />
            <ArrowDownIcon className={`h-3 w-3 -mt-1 transition-all ${sortOrder === "desc" ? "text-primary scale-110" : "text-base-content/30"}`} />
          </div>
        )}
      </div>
    </th>
      ...
      <div className="bg-base-100 rounded-2xl shadow-md border border-base-200 p-4 mb-3 hover:shadow-lg transition-all duration-300">
        <div className="flex justify-between items-start mb-3">
          <div className="flex items-center gap-3 flex-1">
            <div className={`w-3 h-10 bg-gradient-to-b ${categoryConfig.gradient} rounded-full`} />
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-base-content/50 mt-0.5">By: {getPaidByName(spend.paidBy)}</p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3 mb-3">
          <div className="flex items-center gap-2">
            ...
            <span className={getAmountColor(spend.amount)}>{formatINR(spend.amount)}</span>
          </div>
          <div className="flex items-center gap-2">
            <CalendarIcon className="h-4 w-4 text-primary" />
            <span className="text-sm font-medium text-base-content">{formatDate(spend.spendDate)}</span>
          </div>
        </div>
        ...
        <div className="flex justify-between pt-3 border-t border-base-200">
          <button onClick={() => onView(spend)} className="flex items-center gap-2 px-3 py-2 text-primary hover:bg-primary/10 rounded-xl transition-all duration-200 text-sm font-medium">
            <EyeIcon className="h-4 w-4" /> View
          </button>
          <button onClick={() => onEdit(spend)} className="flex items-center gap-2 px-3 py-2 text-success hover:bg-success/10 rounded-xl transition-all duration-200 text-sm font-medium">
            <PencilIcon className="h-4 w-4" /> Edit
          </button>
          <button onClick={() => onDelete(spend._id)} className="flex items-center gap-2 px-3 py-2 text-error hover:bg-error/10 rounded-xl transition-all duration-200 text-sm font-medium">
    ...
    <div className="bg-base-100/80 backdrop-blur-sm rounded-2xl sm:rounded-3xl shadow-md border border-base-200 overflow-hidden">
      <div className="p-4 sm:p-6">
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b-2 border-base-200 bg-gradient-to-r from-base-200 to-primary/10">
                {headerCell("Title",    "title",     "w-2/5")}
                {headerCell("Amount",   "amount",    "w-1/6")}
                {headerCell("Date",     "spendDate", "w-1/6")}
            ...
            <tbody className="divide-y divide-base-200">
              {items.map((spend) => {
                ...
                const statusConfig   = getStatusConfig(spend.status);
                return (
                  <tr key={spend._id} className="group hover:bg-primary/5 transition-all duration-300">

                    {/* Title */}
                    <td className="py-4 px-4">
                      <div className="flex items-start space-x-3">
                        <div className={`w-2 h-12 bg-gradient-to-b ${categoryConfig.gradient} rounded-full flex-shrink-0`} />
                    ...
                    <td className="py-4 px-4">
                      <div className="flex items-center gap-2 text-base-content/70 bg-base-200 rounded-lg px-3 py-2">
                        <CalendarIcon className="h-4 w-4 text-primary" />
                        <span className="text-sm font-medium">{formatDate(spend.spendDate)}</span>
                      </div>
                    </td>
                    ...
                    <td className="py-4 px-4">
                      <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-all duration-300 transform translate-x-2 group-hover:translate-x-0">
                        <button onClick={() => onView(spend)} className="p-2 text-primary hover:bg-primary/10 rounded-xl transition-all duration-200 hover:scale-110 active:scale-95" title="View">
                          <EyeIcon className="h-4 w-4" />
                        </button>
                        <button onClick={() => onEdit(spend)} className="p-2 text-success hover:bg-success/10 rounded-xl transition-all duration-200 hover:scale-110 active:scale-95" title="Edit">
                          <PencilIcon className="h-4 w-4" />
                        </button>
                        <button onClick={() => onDelete(spend._id)} className="p-2 text-error hover:bg-error/10 rounded-xl transition-all duration-200 hover:scale-110 active:scale-95" title="Delete">
        ...
        <div className="lg:hidden space-y-3">
          {items.map((spend) => (
            <MobileSpendCard key={spend._id} spend={spend} />
          ))}
        </div>
      ...
      {items.length > 0 && (
        <div className="bg-base-200/50 border-t border-base-200 px-4 sm:px-6 py-3">
          <div className="flex justify-between items-center text-sm text-base-content/70">
            <span>Showing <span className="font-semibold text-primary">{items.length}</span> spends</span>
